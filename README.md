...
- **📂 打开 / 🗑 删除按钮**：每行操作列，起本地服务后点 📂 在项目目录打开终端并**自动恢复对应对话**（`claude --resume` / `codex resume`），点 🗑 删除会话（进回收站）；file:// 双击打开为只读模式，按钮降级为复制恢复命令
...
- **📂 打开 / 🗑 删除**：file:// 双击打开是**只读模式**——页面顶部出现橙色提示条，打开按钮降级为复制恢复命令、删除按钮降级为复制回收站命令；起 `scripts/server.py` 后访问 `http://localhost:8123/`，打开按钮在**项目目录下打开终端并自动执行恢复命令进入对应对话**（`claude --resume` / `codex resume`，优先 Windows Terminal，兜底 cmd），删除按钮弹确认框后真删（进回收站可恢复）
...
- **📂 打开** → `POST /api/open`，在**项目目录下打开终端并自动执行恢复命令进入对应对话**（`claude --resume <sid>` / `codex resume <sid>`，优先 Windows Terminal，兜底 cmd）
