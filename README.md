...
> HelloXW 出品 · 网页只读 · 起 `scripts/server.py` 后『打开/删除』按钮生效，删除进回收站可恢复
...
- **📂 打开 / 🗑 删除按钮**：每行操作列，起本地服务后打开项目目录 / 删除会话（进回收站）
...
- **📂 打开 / 🗑 删除**：file:// 双击打开时删除降级为复制命令；起 `scripts/server.py` 后访问 `http://localhost:8123/`，打开按钮用资源管理器打开项目目录，删除按钮弹确认框后真删（进回收站可恢复）
